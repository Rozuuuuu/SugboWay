...
func CalculateBPRCost(distanceMeters float64, passengerVolume int, roadType string, departureTime time.Time) float64 {
	...
	if roadType == "national" {
		speedKph = 40.0
		capacity = 10000.0 // Assumed daily capacity for national corridor
	} else {
		// default to barangay road
		speedKph = 20.0
		capacity = 5000.0 // Assumed daily capacity for barangay roads
	}

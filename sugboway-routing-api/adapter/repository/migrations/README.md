...
### Making every scraped route reachable

`/route/shape` only returns a shape that a trip links to. `0004_weebly_routes_trips.sql`
(generated by `gen_routes_trips.py` from the committed KMZs) adds a `routes` row
and a `trips` row — with `shape_id` pre-linked — for each scraped code, so all
54 routes render, not just the originally seeded 8. Existing routes keep their
metadata (`ON CONFLICT DO NOTHING`).

**Apply order:** `schema.sql` → `seed_*.sql` → `0003_weebly_real_shapes.sql`
→ `0004_weebly_routes_trips.sql`.

```bash
python gen_routes_trips.py --out adapter/repository/migrations/0004_weebly_routes_trips.sql
```
